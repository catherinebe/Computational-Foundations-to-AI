...
* Y is the true value data generated as 𝑁-dimensional centered response vector 𝐲
...
Illustrate the effect of the tuning parameter on the inferred ridge regression coefficients by generating a plot of nine lines (one for each of the 𝑝 = 9 features), with the 𝑦-axis as $𝛽_𝑗 = 1,2, ... ,9$ , and the 𝑥-axis the corresponding log-scaled tuning parameter value $\log_{10}(𝜆)$ that generated the particular $\hat{𝛽}_j$. Label both axes. Without the log scaling of the tuning parameter, the plot will look distorted.
"""
...
* Y_d2 is the true values data (i.e. Credit Balances) generated as 𝑁-dimensional centered response vector 𝐲
...
Illustrate the effect of the tuning parameter on the cross validation error by generating a plot  with the 𝑦-axis as $CV_{(5)}$ error, and the 𝑥-axis the corresponding log-scaled tuning parameter value $\log_{10}(𝜆)$ that generated the particular $CV_{(5)}$ error. Label both axes. Without the log scaling of the tuning parameter, the $CV_{(5)}$ plot will look distorted.
"""
...
Indicate the value of 𝜆 that generated the smallest $CV_{(5)}$ error.
"""
